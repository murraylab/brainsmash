...
# TODO absorb this into test_sampled once tested
def test_smash_sampled_variogram_fits(
        brain_map, distmat, index, nsurr=10, include_naive=False, **params):
    """
    Test variogram fits for :class:`brainsmash.maps.core.Smash` with strategy
    Sampled.
    ...
    # Compute empirical variogram
    v = generator.compute_variogram(generator.x_, idx)
    u = generator.distmat_[idx, :]
    umax = np.percentile(u, generator.umax_)
    uidx = np.where(u < umax)
    ...
    # Compute surrogate map variograms
    surr_var = np.empty((nsurr, generator.nbins_))
    for i in range(nsurr):
        v_null = generator.compute_variogram(surrogate_maps[i], idx)
        surr_var[i] = generator.smooth_variogram(
            u=u[uidx], v=v_null[uidx], return_u0=False)
    ...
    if include_naive:
        naive_surrs = np.array(
            [np.random.permutation(generator.x_) for _ in range(nsurr)])
        naive_var = np.empty((nsurr, generator.nbins_))
        for i in range(nsurr):
            v_null = generator.compute_variogram(naive_surrs[i], idx)
            naive_var[i] = generator.smooth_variogram(
                u=u[uidx], v=v_null[uidx], return_u0=False)
