...
def import_cifti_mapping(surfaces=True, volumes=True):
    ...
    try:
        ...
        raise IOError(
            "Check that lib.utils.export_cifti_mapping is saving the"
            " expected output files -- it likely isn't.")
    return maps


def export_cifti_mapping(surfaces=True, volumes=True):
    ...
    ----------
    surfaces : bool
        export the mapping from CIFTI index to surface vertex index for both
        cortical surfaces (i.e. left and right hemispheres)
    volumes : bool
        export the mapping form CIFTI index to voxel (i, j, k) and
        anatomical structure name
    ...
    if not (surfaces or volumes):
        return

    filenames = list()

    cmd = "wb_command -cifti-export-dense-mapping '{}' COLUMN ".format(
        files.parcel_labels_lr)

    if volumes:
        filenames.append(files.volume_map_file)
        cmd += " -volume-all '{}' -structure ".format(
            files.volume_map_file)

    if surfaces:
        filenames += [files.cortex_left_map_file,
                      files.cortex_right_map_file]
        cmd += "-surface CORTEX_LEFT '{}' -surface CORTEX_RIGHT " \
               "'{}'".format(files.cortex_left_map_file,
                             files.cortex_right_map_file)
